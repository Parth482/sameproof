...
export function PassportCard({ passport, shareUrl, onRefresh, publicView = false }: { passport: PublicPassport; shareUrl?: string; onRefresh?: () => void; publicView?: boolean }) {
  ...
  const policyName = passport.policySnapshot.name.replace(/\s*[-–—]\s*prototype interpretation$/i, "");
  const competitorSeller = competitor.sellerName?.value ? String(competitor.sellerName.value) : null;
  const presentation = present ? <dialog ref={presentationRef} aria-labelledby="counter-view-title" onCancel={(event) => { event.preventDefault(); closePresentation(); }} className="fixed inset-0 z-50 m-0 h-dvh w-full max-w-none overflow-y-auto border-0 bg-slate-900 p-3 text-white backdrop:bg-slate-900 sm:p-8">
    <article className="mx-auto flex min-h-full max-w-3xl flex-col overflow-hidden rounded-[1.75rem] border border-white/15 bg-slate-900 shadow-2xl">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-5 py-8 text-center sm:px-10"><span className="grid size-16 place-items-center rounded-full bg-emerald-500 text-white"><BadgeCheck size={34} /></span><p className="mt-5 text-xs font-bold uppercase tracking-[.2em] text-emerald-300">{competitorSeller ? `Available at ${competitorSeller}` : "Show this to the store"}</p><h2 id="counter-view-title" className="mt-3 text-3xl font-semibold tracking-[-.04em] sm:text-5xl">{String(model)}</h2><p className="mt-1 text-sm text-slate-400">{policyName}</p><div className="mt-7 rounded-3xl bg-white px-7 py-6 text-slate-950 shadow-xl"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">{competitorSeller ? `Get it from ${competitorSeller} for` : "Price to request"}</p><p className="mt-1 text-5xl font-semibold tracking-[-.06em] text-blue-700">{moneyOrUnknown(matchedPrice)}</p>{saving && saving.amountCents > 0 && <p className="mt-2 font-bold text-emerald-700">Save {formatMoney(saving)}</p>}</div><div className="mt-6 grid w-full max-w-xl gap-2 sm:grid-cols-3"><CounterCheck label="Same product" value={decision.identity.status === "exact" ? "Confirmed" : "Check needed"} /><CounterCheck label="Policy" value={`${passedPolicy}/${decision.policyChecks.length} pass`} /><CounterCheck label="Valid for" value={expired ? "Expired" : countdown} /></div><p className="mt-7 max-w-xl text-sm leading-6 text-slate-300">{competitorSeller ? `Show this alongside ${competitorSeller}\u2019s page. The store decides.` : "Show this alongside the competitor\u2019s page. The store decides."}</p></div>
    </article>
  </dialog> : null;
  ...
  return <>{presentation}<article className={`mx-auto overflow-hidden rounded-[1.5rem] border bg-white shadow-xl ${expired ? "border-amber-300" : "border-slate-200"}`}>
    <header className="border-t-4 border-blue-500 bg-slate-900 p-5 text-white sm:p-7">
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">{expired && onRefresh ? <button type="button" className="button-primary" onClick={onRefresh}><RotateCcw size={17} />Refresh</button> : !expired ? <button ref={presentButtonRef} type="button" className="button-primary" onClick={() => setPresent(true)}><BadgeCheck size={17} />Where to buy</button> : null}<button type="button" className="button-secondary" onClick={share}><Share2 size={17} />Share</button></div><p className="mt-2 text-xs text-slate-300" aria-live="polite">{shareMessage}</p>
    </header>
