...
export function PassportCard({ passport, shareUrl, onRefresh, publicView = false }: { passport: PublicPassport; shareUrl?: string; onRefresh?: () => void; publicView?: boolean }) {
  ...
  const presentation = present ? <dialog ref={presentationRef} aria-labelledby="counter-view-title" onCancel={(event) => { event.preventDefault(); closePresentation(); }} className="fixed inset-0 z-50 m-0 h-dvh w-full max-w-none overflow-y-auto border-0 bg-slate-950 p-3 text-white backdrop:bg-slate-950 sm:p-8">
    <article className="mx-auto flex min-h-full max-w-3xl flex-col overflow-hidden rounded-[1.75rem] border border-white/15 bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,.45),transparent_45%),linear-gradient(145deg,#0f172a,#020617)] shadow-2xl">
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-5 py-8 text-center sm:px-10"><span className="grid size-16 place-items-center rounded-full bg-emerald-400 text-slate-950 shadow-[0_0_45px_rgba(52,211,153,.35)]"><BadgeCheck size={34} /></span><p className="mt-5 text-xs font-bold uppercase tracking-[.2em] text-emerald-300">Ready for retailer review</p><h2 id="counter-view-title" className="mt-3 text-3xl font-semibold tracking-[-.04em] sm:text-5xl">{String(model)}</h2><div className="mt-7 rounded-3xl bg-white px-7 py-6 text-slate-950 shadow-xl"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">Price to request</p><p className="mt-1 text-5xl font-semibold tracking-[-.06em] text-blue-700">{moneyOrUnknown(matchedPrice)}</p>{saving && saving.amountCents > 0 && <p className="mt-2 font-bold text-emerald-700">Potential saving {formatMoney(saving)}</p>}</div><div className="mt-6 grid w-full max-w-xl gap-2 sm:grid-cols-3"><CounterCheck label="Same product" value={decision.identity.status === "exact" ? "Confirmed" : "Check needed"} /><CounterCheck label="Policy" value={`${passedPolicy}/${decision.policyChecks.length} pass`} /><CounterCheck label="Evidence expires in" value={expired ? "Expired" : countdown} /></div><p className="mt-7 max-w-xl text-sm leading-6 text-slate-300">Show this screen with the original competitor page. SameProof explains the evidence; the retailer makes the final decision.</p></div>
    </article>
  </dialog> : null;
  ...
  return <>{presentation}<article className={`mx-auto overflow-hidden rounded-[1.5rem] border bg-white shadow-xl ${expired ? "border-amber-300" : "border-slate-200"}`}>
    <header className="bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,.38),transparent_48%),linear-gradient(145deg,#0f172a,#020617)] p-5 text-white sm:p-7">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[.15em] text-blue-200"><ShieldCheck size={17} />SameProof passport</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.04em] sm:text-3xl">{expired ? "Evidence needs a fresh check" : "Ready to show the retailer"}</h2><p className="mt-1 text-sm text-slate-300">{String(model)}</p></div>{expired ? <span className="inline-flex items-center gap-2 rounded-full border border-amber-700 bg-amber-950 px-3 py-1.5 text-xs font-bold text-amber-100"><TriangleAlert size={15} />Needs refresh</span> : <DecisionBadge status={decision.status} large />}</div>

      <div className="mt-5 rounded-2xl bg-white p-4 text-slate-950 shadow-xl sm:p-5"><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">Price to request</p><p className="mt-1 text-4xl font-semibold tracking-[-.055em] text-blue-700">{moneyOrUnknown(matchedPrice)}</p><div className="mt-4 grid grid-cols-2 gap-3 border-t border-slate-200 pt-4 sm:grid-cols-3"><PriceFact label="Retailer price" value={decision.pricing.targetPrice} /><PriceFact label="Competitor delivered" value={decision.pricing.comparableCompetitorTotal} /><div className="col-span-2 rounded-xl bg-emerald-50 px-3 py-2.5 sm:col-span-1"><p className="text-[11px] font-semibold text-emerald-700">Potential saving</p><p className="mt-0.5 text-lg font-bold text-emerald-800">{saving ? formatMoney(saving) : "Unknown"}</p></div></div></div>

      <div className={`mt-4 flex items-center gap-3 rounded-xl px-4 py-3 ${expired || remaining < 10 * 60_000 ? "bg-amber-950" : "bg-white/10"}`} role="timer"><Clock3 size={19} /><div><p className="text-[11px] font-bold uppercase tracking-[.12em]">{expired ? "Evidence is no longer current" : "Evidence expires in"}</p><p className="mt-0.5 font-mono text-lg font-semibold">{expired ? "Recheck price and stock" : countdown}</p></div></div><p className="sr-only" aria-live="polite">{timeAnnouncement}</p>

      <div className="mt-4 flex flex-col gap-2 sm:flex-row">{expired && onRefresh ? <button type="button" className="button-primary" onClick={onRefresh}><RotateCcw size={17} />Recheck price and stock</button> : !expired ? <button ref={presentButtonRef} type="button" className="button-primary" onClick={() => setPresent(true)}><BadgeCheck size={17} />Show at counter</button> : null}<button type="button" className="button-secondary" onClick={share}><Share2 size={17} />Share passport</button></div><p className="mt-2 text-xs text-slate-300" aria-live="polite">{shareMessage}</p>
    </header>
    ...
    <div className="space-y-3 p-4 sm:p-6">
      <p className="px-1 text-xs font-bold uppercase tracking-[.14em] text-slate-500">Proof behind the result</p>
    ...
    <footer className="border-t border-slate-200 bg-slate-50 p-4 sm:p-6"><p className="text-xs leading-5 text-slate-600">This is explainable prototype guidance, not guaranteed approval. Final approval remains with the retailer.</p><details className="group mt-2"><summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 text-xs font-semibold text-slate-500"><Fingerprint size={15} />Technical integrity details</summary><p className="mt-1 flex items-center gap-2 text-xs"><Fingerprint size={15} className={integrity === "valid" ? "text-emerald-600" : "text-slate-500"} />Integrity {integrity === "checking" ? "checking…" : integrity}</p><p className="mt-1 break-all font-mono text-[10px] leading-4 text-slate-500">SHA-256 {passport.integrityHash}</p></details>{!publicView && onRefresh && !expired && <button type="button" className="button-ghost mt-2 px-2 text-xs" onClick={onRefresh}><RotateCcw size={15} />Recheck evidence</button>}</footer>
  </article></>;
}
...
function CounterCheck({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/10 p-3"><p className="text-[11px] font-semibold text-slate-300">{label}</p><p className="mt-1 font-bold text-white">{value}</p></div>;
}
