...
export function PassportClient({ comparisonId }: { comparisonId: string }) {
  ...
  return <div className="mx-auto max-w-2xl"><section className="surface overflow-hidden"><div className="bg-amber-50 p-6 text-center sm:p-8"><ShieldCheck className="mx-auto text-amber-700" size={30} /><h2 className="mt-4 text-2xl font-semibold tracking-[-.035em]">A current eligible result is needed</h2><p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-700">Return to the result and run the latest policy check. SameProof creates the passport automatically once it is ready.</p>{error && <div className="mt-5 text-left"><ErrorState message={error.message} requestId={error.requestId} onRetry={ready ? generate : undefined} /></div>}<Link href={`/identity-bridge/${comparisonId}`} className="button-primary mt-6"><ArrowLeft size={17} />Return to result</Link></div></section></div>;
}
