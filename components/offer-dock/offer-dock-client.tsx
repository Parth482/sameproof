      ...
      <div className="space-y-4 lg:space-y-5">
        <details className="surface group overflow-hidden" open={evidenceEditorOpen} onToggle={(event) => setEvidenceEditorOpen(event.currentTarget.open)}>
          ...
          <div className="p-5 sm:p-6">
            ...
            {activeDraft.tab === "url" && <div><label className="text-sm font-semibold" htmlFor={`${activeSide}-retailer-url`}>Retailer product URL</label><div className="mt-2 flex flex-col gap-2 sm:flex-row"><input id={`${activeSide}-retailer-url`} className="field flex-1" type="url" placeholder="https://retailer.example/monitor" value={activeDraft.url} onChange={(event) => updateDraft(activeSide, { url: event.target.value, urlResolved: null })} /><button type="button" className="button-secondary shrink-0" onClick={resolveUrl}><ExternalLink size={17} />Check URL</button></div>{activeDraft.urlResolved === true && <p className="mt-3 rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800" role="status">Known offer found. Confirm the imported facts on the next screen.</p>}{activeDraft.urlResolved === false && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm leading-6 text-amber-900" role="status">This page is not in our catalogue. Enter only the facts you can see; everything else will remain Unknown.</p>}</div>}
